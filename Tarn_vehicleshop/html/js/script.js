    ...
    if (data.testcar) {
        ...
        timetestcarme = setInterval(function() {
            ...
            if (timeleft == 0) {
                ...
                $.post('http://Tarn_vehicleshop/timeouttest');
            }
            timeleft--;
        }, 1000);
    }
...
$(document).keyup(function(e) {
    if (e.key === 'Escape') {
        ...
        $("#popup-buycar").remove();
        $("#box-information").remove();
        $.post('http://Tarn_vehicleshop/quit');
    }
})
...
function Choosecar(carname,model,pricecar,kg,classcar) {
    ...
    $.post('http://Tarn_vehicleshop/choosecar', JSON.stringify({
        model: model,
    }));
...
function BUYCAR() {
    ...
    if (choosepayment === 'money') {
        $.post('http://Tarn_vehicleshop/buycar', JSON.stringify({
            color1: color_1,
            color2: color_2,
